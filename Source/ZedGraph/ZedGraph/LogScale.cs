	...
	#region methods
		...
		override internal double CalcBaseTic()
		{
			...
			else
			{
				...
				return Math.Ceiling( Scale.SafeLog( _base, _min ) - 0.00000001 );
			}
		...
		override internal int CalcNumTics()
		{
			...
			nTics = (int)( ( Scale.SafeLog( _base, _max ) - Scale.SafeLog( _base, _min ) ) / CyclesPerStep ) + 1;
		...
		override public void PickScale( GraphPane pane, Graphics g, float scaleFactor )
		{
			...
			if ( _minAuto )
				_min = Math.Pow( (double) _base,
					Math.Floor( Math.Log( _min )/Math.Log(_base) ) );
			if ( _maxAuto )
				_max = Math.Pow( (double) _base,
					Math.Ceiling( Math.Log( _max )/Math.Log(_base) ) );
		...
		override internal string MakeLabel( GraphPane pane, int index, double dVal )
		{
			...
			if ( _isUseTenPower )
				...
				return Math.Pow( _base, dVal ).ToString( _format );
		}
